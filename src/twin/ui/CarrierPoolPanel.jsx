  ...
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', padding: '8px 12px', borderBottom: '1px solid #1e293b' }}>
        ...
        <button onClick={onClose} title="Close carrier editor" aria-label="Close carrier editor" style={closeBtn}>×</button>
      </div>
      ...
      <div style={{ padding: '10px 12px' }}>
        {rows.length === 0 ? (
          ...
          rows.map((d, idx) => (
            ...
            >
              <div style={{ fontSize: 12, fontFamily: T.mono, color: '#94a3b8', marginBottom: 4 }}>
                {d.id} <span style={{ color: '#475569' }}>· {d.carrier_kind}</span>
              </div>
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                <Field label="fleet size" testid={`pool-count-${d.id}`} value={d.count}
                  disabled={!editing} onChange={(v) => updateDraft(idx, { count: v })} />
                <Field label="units/trip" testid={`pool-pertrip-${d.id}`} value={d.units_per_trip}
                  disabled={!editing} onChange={(v) => updateDraft(idx, { units_per_trip: v })} />
                <Field label="load/unload (s)" testid={`pool-loadunload-${d.id}`} value={d.load_unload_seconds}
                  disabled={!editing} onChange={(v) => updateDraft(idx, { load_unload_seconds: v })} />
                <Field label="loaded speed (m/min)" testid={`pool-loaded-${d.id}`} value={d.speed_loaded_m_per_min}
                  disabled={!editing} onChange={(v) => updateDraft(idx, { speed_loaded_m_per_min: v })} />
                <Field label="empty speed (m/min)" testid={`pool-empty-${d.id}`} value={d.speed_empty_m_per_min}
